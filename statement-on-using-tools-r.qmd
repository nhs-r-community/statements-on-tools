...
|-------------------|-------------------|-----------------------------------|
| Code | Allows the user to use functions in their own code | ![Screenshot of code for the ptd_spc() function used to create SPC charts from the {NHSplotthedots} package](img/ptd_spc_image.png){height="100"} |
| Data | Demonstrates the function(s), training new users | ![Screenshot of 'storms' dataset from {dplyr}](img/storms_data_image.png){height="100"} |
| Documentation | User guides, package vignettes and other documentation | ![Documentation for the ptd_spc() function from {NHSRplotthedots}](img/ptd_spc_docs_image.png){height="100"} |

Screenshot of code for the ptd_spc() function used to create SPC charts from the {NHSplotthedots} package And we can usually find at least three of these components in most R packages (tests are a bit more elusive, and are less commonly encountered). It's also worth noting that the data included in community packages is sample data or open data that is really meant to help users experiment with the new functions in a package. It's definitely not standard practice to share real data for analysis in this way. Lots of these standard datasets built into packages - like [mtcars](https://www.rdocumentation.org/packages/datasets/versions/3.6.2/topics/mtcars) - are used over and over again as ways of demonstrating R functions.
