...
::: aside
The Quarto CLI download is a `win.msi` file so many not require authorisation from IT teams to download and install.
:::
...
The package {installr} has a function `installr::uninstall.r()` which may not required admin rights.
